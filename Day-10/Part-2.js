...
function followTrail(input, width, height, x, y, curr, path) {
  if (curr === 9) {
    // print(input, path);
    return 1;
  }
  ...
  for (let nextVec of nexts) {
    ...
    if (
      ...
      if (input[ny][nx] === nextVal) {

        const nPath = [ ...path, { x: nx, y: ny  }];
        trailCountFromHere +=
          followTrail(input, width, height, nx, ny, nextVal, nPath);
      }
    }
  }
...
for (let y=0; y<height; y++) {
  for (let x=0; x<width; x++) {
    if (input[y][x] === 0) {
      const localTotal = followTrail(input, width, height, x, y, 0, [{ x, y }]);
